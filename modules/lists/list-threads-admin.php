...
$cols = "u.Id as UserId,th.Id as thread_id,th.UserId as user_id,th.upload_post_id,th.Image,th.Message,th.Likes,th.Category,th.CreateDate";
$db->join("users u", "u.Id=th.UserId", "LEFT");
$db->where("u.EmployeeGroup", 1);
$db->where('th.WallType', 3);
$db->where('th.Category', $_GET['category']);
$db->orderBy('th.CreateDate', 'DESC');
$threadsData = $db->get("thread th", null, $cols);
...
//      if (in_array($val['Type'], $typearray) && $val['Msg_id'] == $threadData['thread_id']){
//          echo "<pre>";print_r($val['Msg_id'] );
//          echo "<pre>";print_r($threadData['thread_id']);
//      }
//    }
//  }
// exit;
...
<ul>
   <?php foreach ($threadsData as $threadData)
    {
        // echo "<pre>";print_r($threadData);exit;
        $i = 0;
      ...
      $db->where('thread_id',$threadData['thread_id']);
...
?>
   <div class="chat-thread">
      ...
      <div class="chat-thread-content" id="message_<?php echo $threadData['thread_id']; ?>">
         <p class="message-content edit_post"><?php echo $return_msg; ?></p>
         <?php if (!empty($threadData['Image']))
            {
                echo "<img src='" . $threadData['Image'] . "' />";
            } ?>
           <?php if ($threadData['user_id'] == $_SESSION['UserId']){ ?>
          <div class="edit_post_div">
            <i class="glyphicon glyphicon-pencil btnEdit" onClick="showEditArea(this, <?php echo $threadData['thread_id']; ?>)"></i>
            <i class="glyphicon glyphicon-trash btnDelete" onClick="callCrudAction('delete_post',<?php echo $threadData['thread_id']; ?>)"></i>
         </div>
         <?php
            } ?>
      </div>
      <div class="image-list-admin">
         <?php
            $i = 0;
            foreach ($chatSorted as $key => $value)
            {
                if (in_array($value['Type'], $typearray) && ($value['Msg_id'] == $threadData['thread_id']) && $value['Msg_type'] == '1')
                {
?>
         <div id="image-list-admin">
         ...
         <div id="image-list-admin">
            ...
            <?php if ($_SESSION['EmployeeGroup'] == 1 && ($threadData['user_id'] == $_SESSION['UserId']))
                    { ?>
              <!-- <div class="chatlist-text"><a  class="md-trigger delete_post" data-modal="modal-upload-media" onclick="crudModalUpload(5,<?php echo $value['Id']; ?>,1)">Slet</a></div> -->
            <?php
                    } ?>
      ...
      <div class="chat-thread-meta">
         <ul class="magic-number-list">
            <li>
            <!-- <input type="range" min="1" max="100" value="1" class="like-slider" id="threadAdminLike" onchange="likeThreadNew(this.value, <?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)"/> -->
             <div id="admin-likes-tooltip_<?php echo $threadData['thread_id']; ?>1" class="tip-content">
                     <a onclick="likeThreadNew(7,<?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)" >7</a>
                     <a onclick="likeThreadNew(17,<?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)" >17</a>
                     <a onclick="likeThreadNew(27,<?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)" >27</a>
                     <a onclick="likeThreadNew(47,<?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)" >47</a>
                     <a onclick="likeThreadNew(57,<?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)" >57</a>
                  </select>
                  </div>
                <a class="hover-likes tip" data-toggle="tooltip"  data-tip="admin-likes-tooltip_<?php echo $threadData['thread_id']; ?>" rel="tooltip" href="#">
               <!-- <a onclick="likeThread(<?php echo $threadData['thread_id']; ?>,<?php echo $threadData['Category']; ?>,<?php echo $_SESSION['UserId']; ?>)"> -->
                  <?php // if (in_array($_SESSION['UserId'], $threadLikes))
                  if ($likes > 0)
                     { ?>
                  <i>
             ...
             <li class="tooltip-wrapper">
                <ul class="emoji-icon-container">
                  <?php  foreach ($emojiArray as $icon) { ?>
                      <li><img src="/assets/images/reaction-icons/<?= $icon; ?>.png" class="emoji-icon" data-emoji-rating="<?= $threadData['thread_id']; ?>" onclick="reactionThread('<?= $icon; ?>',<?= $threadData['thread_id']; ?>,<?= $threadData['Category']; ?>,<?= $_SESSION['UserId']; ?>)" /></li>
                  <?php   }   ?>
                  </ul>
      ...
      <?php
            $db->where('ThreadId', $threadData['thread_id']);
...
?>
                  ...
                  <?php
                        ...
                        foreach ($chatSorted as $key => $val)
                        {
                          ...
                          <div id="image-list-admin">
                             ...
                             <?php if ($_SESSION['EmployeeGroup'] == 1 && ($threadmessageData['UserId'] == $_SESSION['UserId']))
                                { ?>
      ...
      <div class="chat-write chat-write-comment">
         ...
         <div class="add_comment">
            <input class="chat-input-write" id="chat_comment" data-emojiable="true" data-emoji-input="unicode" data-thread="<?php echo $threadData['thread_id']; ?>" placeholder="Din kommentar..." />
            <input type="file" data-id="<?php echo $threadData['thread_id']; ?>" name="comment_photo" id="comment_images" class="comment-pin" accept="image/*,.mp4,.avi,.mkv">
                <i class="glyphicon glyphicon-paperclip upload-pin"></i>
                <input type="hidden" id="comment_id" name="comment_id" class="comment_id" value="" data-cls="preview_<?php echo $threadData['thread_id']; ?>">
            <span class="btn btn-cta btn-send-comment" data-id="btn-send-comment" data-thread="<?php echo $threadData['thread_id']; ?>">Send</span>
         </div>
      </div>
      <div class="progress-wrapper commentMain_<?php echo $threadData['thread_id']; ?>" id="main-progress" style="display:none; position:relative; top:15px;" >
      ...
      <div class="preview-commentpic" id="preview_<?php echo $threadData['thread_id']; ?>">
         <div class="preview_<?php echo $threadData['thread_id']; ?>" style="position: relative; left: 60px;">
...
$('.comment-pin').on('change',function(e){
   ...
   $('.image-list-admin').slick({
    ...
    $('.image-list-admin').slickLightbox({
    src: 'src',
