		...
		userFilesDropzone: {
			...
			initFileUploader: function(base, target) {
				...
				var onyxDropzone = new Dropzone(target, {
					url: ($(target).attr("action")) ? $(target).attr("action") : "/actions/ajax/threadEdit.php", // Check that our form has an action attr and if not, set one here
	                maxFiles: 1,
					maxFilesize: 1000,
				...
				onyxDropzone.on('removedfile', function (file) {
					...
					$.ajax({
						type: "POST",
						url: ($(target).attr("action")) ? $(target).attr("action") : "/file-upload-media.php",
						data: {
				...
				onyxDropzone.on("queuecomplete", function(file) {
